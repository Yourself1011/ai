...
class Attention(Layer):
    def __init__(
        ...
        self.b: npt.NDArray = np.zeros((embedDim), dtype=np.float32)
        ...
        self.bError: npt.NDArray = np.zeros((embedDim), dtype=np.float32)
    ...
    def backProp(self, error: npt.NDArray):
        ...
        self.bError += error.astype(np.float16).sum(0).sum(0)
    ...
    def normalizeError(self, batchSize: int):
        self.qkvError = f16clamp(self.qkvError) / batchSize
        self.projError = f16clamp(self.projError) / batchSize
    ...
    def gradientDescent(self, learningRate: float, t: int, mult: float):
        ...
        self.bError: npt.NDArray = np.zeros((self.embedDim))
